# usage: python3 calltracer_desc_01.py
...
class tracer(object):  # a decorator + descriptor
    ...
    def __get__(self, instance, owner):  # runs on method attribute lookup
        # print('self: %s\ninstance: %s\nowner %s\n' % (self, instance, owner))
        return wrapper(self, instance)  # returns a wrapper instance


class wrapper:
    # constructor saves both instances... to route calls back through the
    # decorator/descriptor
    def __init__(self, desc, subj):
        self.desc = desc  # a tracer object
        self.subj = subj  # a person object

    def __call__(self, *args, **kwargs):
        # runs tracerobj.__call__(personobj, *args, **kwargs)
